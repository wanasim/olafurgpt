...
function SourceNumberButton({ index }: { index: number }) {
  return (
    <div className="text-xs w-5 h-5 rounded-full bg-gray-100 mb-2 flex items-center justify-center text-blue-950 font-semibold hover:text-white hover:bg-primary hover:cursor-pointer">
      {index + 1}
    </div>
  );
}
...
export function ChatSources({ data }: { data: SourceData }) {
  const sources: NodeInfo[] = useMemo(() => {
    ...
    console.log("DATAAnodes", data.nodes);
    data.nodes
      .filter((node) => (node.score ?? 1) > SCORE_THRESHOLD)
      .sort((a, b) => (b.score ?? 1) - (a.score ?? 1))
      .forEach((node) => {
        const url = node.url || node.text.match(/(https?:\/\/[^\s"'>]+)/g)?.[0];
        const nodeInfo = {
          id: node.id,
          url,
        };
  ...
  return (
    <div className="space-x-2 text-sm">
      <span className="border-l-2 border-indigo-400 pl-2 font-semibold">
        Sources:
      </span>
            ...
            nodeInfo.url && (
              <div key={nodeInfo.id}>
                <HoverCard>
                  ...
                  <HoverCardContent className="w-[320px] text-wrap text-ellipsis">
